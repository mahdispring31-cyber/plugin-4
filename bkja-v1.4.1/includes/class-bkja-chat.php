...
class BKJA_Chat {
    ...
    protected static function build_high_income_response( $items ) {
        ...
        if ( empty( $items ) ) {
            ...
            $lines[] = '• خدمات تخصصی با مجوز یا مهارت عمیق (پزشکی، دندانپزشکی، زیبایی حرفه‌ای).';
            $lines[] = '• پیمانکاری/ساختمان و پروژه‌های اجرایی با تیم کوچک یا مدیریت پروژه.';
            $lines[] = '• تکنولوژی و فریلنس دلاری (توسعه نرم‌افزار، طراحی محصول، مارکتینگ عملکردی).';
            $lines[] = '🧾 جمع‌بندی: این مسیرها معمولاً ظرفیت درآمد بالاتری دارند، حتی بدون داده مستقیم.';
            $lines[] = 'برای دقیق‌تر شدن: شهر، میزان سرمایه اولیه و زمان آزاد هفتگی‌ات چقدر است؟';
            return implode( "\n", array_filter( array_map( 'trim', $lines ) ) );
        }
        ...
        foreach ( $items as $item ) {
            ...
            $note = '';
            if ( $count < 5 || ! empty( $item['data_limited'] ) ) {
                $note = ' (داده محدود)';
            }
